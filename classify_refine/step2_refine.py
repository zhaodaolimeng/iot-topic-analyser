...
class RfRefiner(object):
    ...
    def run_once(self):
        ...
        # 执行一次聚类
        cluster_label, mu_list = self.kmeans(self.group_p, max_iter=100)
    ...
    @staticmethod
    def kmeans(features, max_iter=1000):

        cluster_label = [0] * len(features)
        mu_list = np.random.rand(len(features), len(features[0]))  # 每个聚类的中心K*27
        ...
        for iter_times in range(max_iter):
            ...
            # 更新mu
            mu_count = [0] * len(features)
            mu_sum = np.zeros((len(features), len(features[0])))
    ...
    输出：
    将改写self.sensor_choice，不进行显式的输出
    """
    def adjust_sensor_choice(self, sensor_proba, choice, difference):

        # TODO 调用nx.max_flow_min_cost求解网络流问题
        """
        sensor_proba为
        """
