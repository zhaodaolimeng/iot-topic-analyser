    ...
    while len(resultlist) != 0:
        for (feedid, datastreamid, time_at, val) in resultlist:
            if old_feedid != '' and \
                ...
                if len(binlist) != 0:
                    ds_dict['feedid'] = feedid
                    ds_dict['dsid'] = datastreamid
                    ds_dict['features'] = binlist
                    
                if feedid != old_feedid:
                    feed_dict[feedid] = ds_dict
                    ds_dict = dict()
...
def save_result(feed_dict):
    labels_true = []
    features = []
    for feed_key, ds_key, flist in feed_dict.items():
        labels_true.append(feed_key + ',' + ds_key)
    ...
    pickle.dump(xively_series, open( "feature3.pickle", "wb" ))
    print('Raw Data Done!')
